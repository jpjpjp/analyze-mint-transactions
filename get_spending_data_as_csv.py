...
'''From an input csv of raw mint transaction data
   extract the transactions relevant to a spending analysis.
   
   Included in this will be adding spending groups (logical
   groupings of transaction categoreis) to each transaction,
   removing spending groups not relevant to a spending analysis, 
   and adjusting any credits so that they appear as "refunds".
...
REMOVED_TRANSACTIONS_REPORT = ec.REPORTS_PATH+'removed-transactions.txt'
...
print('Extracting Spending by year...')
...
for year in df.index.year.unique():        
    ...
    year_df = esd.extract_spending(df, ec.PATH_TO_GROUPS_TO_EXCLUDE, from_date, to_date)
...
all_df.to_csv(ec.PATH_TO_SPENDING_DATA)
# Summarize expenses by category, by year
expenses = all_df.groupby(['Spending Group']).sum()
expenses.to_csv(ec.PATH_TO_SPENDING_BY_GROUP)
