...
def visualize_expenses_by_group(year, year_data, colors, out_file='', spending=True):
    ...
    for idx in year_data.index:
        if year_data.loc[idx] <= 0:
            ...
            if year_data.loc[idx] < 0: 
                print('Spending on ' + idx + ': ${:,.2f}'.format(year_data.loc[idx]) + ' was really income.')
    ...
    year_data.plot(kind='pie', figsize=(8,8), title=title, colors=[colors[key] for key in year_data.index])
...
def visualize_average_spending_by_group(sbg, title, colors, out_file=''):
    ...
    for idx in sbg.index:
        if sbg.loc[idx].Average < 0:
            print('Average spending on ' + idx + ': ${:,.2f}'.format(sbg.loc[idx].Average) + ' is negative. Removing from analysis.')
            average_total -= sbg.loc[idx].Average
            sbg.drop([idx], inplace=True)
        else:
            legend_label.append(idx + ': ${:,.2f}'.format(sbg.loc[idx].Average))
        
    sbg.plot(kind='pie', y='Average', figsize=(8,8), colors=[colors[key] for key in sbg.index], \
...
def read_structured_transactions(structured_transactions, raw_transactions, index, \
    ...
    try:
        ...
        if f1 < f2:
            print('Raw mint transactions data: '+raw_transactions+ \
                ' is newer than the '+structured_data_description+': ' \
                + structured_transactions+'.\n' + \
                'Please run "python get_spending_data_as_csv.py" first.')
            quit()
    except BaseException as e:
        print('Failed to read '+structured_data_description+': '+structured_transactions+ \
            '\n{}'.format(e))
    ...
    try:
        print('Reading '+structured_data_description+' from '+structured_transactions)
